...
class DonateController extends Controller
{
        ...
        try{
            ...
            if ($payment === 'captured') {
                ...
                $payment_table->update([
                    ...
                    'user_name' => $request->name ?? 'Unknown User',
                    'user_email' => $request->email ?? 'unknownuser@gmail.com',
                    'user_mobile' => $request->mobile_no ?? '0000000000',
                ]);
                ...
                // Send emails
                $this->sendEmail($request->email, $r_payment_id,'member');
    ...
    private function sendEmail($email, $r_payment_id, $role)
    {
        try{
            ...
            Mail::to($email)->queue(
                (new SendDonationInvoiceMail($payment_table,$isAdmin,$subject,$count))
                    ->attach($pdfPath)
            );     
